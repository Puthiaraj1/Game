...
class Enemy:
    ...
    def __str__(self):
        return "Name: {0._name}, Lives: {0._lives}, Hit Point: {0._hit_points}".format(self)


class Troll(Enemy): # inherit from Enemy
...
class Vampyre(Enemy):
    ...
    def take_demage(self, damage):
        if not self.dodges():
            super().take_demage(damage=damage)
